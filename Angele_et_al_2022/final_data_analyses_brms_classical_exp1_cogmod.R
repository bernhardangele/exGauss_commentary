...
exp1_data_to_include <- exp1 %>% 
  ...
  mutate(
    ...
    rt = rt # * 1000 # Convert RT to milliseconds # Try seconds instead
  )
...
priors_classical <- c(
  ...
  prior(normal(0, 0.1), class = "b", coef = "PrimeDuration1"),
  prior(normal(0, 0.1), class = "b", coef = "Condition1:PrimeDuration1"),
  
  # Exponential tail tau (response scale, via cogmod's softplus link): Intercept & effects
  prior(normal(0.1, 0.05), class = "Intercept", dpar = "tau"),
  prior(normal(0, 0.05), class = "b", dpar = "tau"),
  
  # Gaussian SD sigma (response scale): Intercept
  prior(normal(0.060, 0.030), class = "Intercept", dpar = "sigma"),

  # Penalize implausibly massive random slope standard deviations (in ms)
  prior(exponential(0.05), class = "sd"),            # Expects random SDs around ~20 ms for mu
  prior(exponential(0.05), class = "sd", dpar = "tau") # Expects random SDs around ~20 ms for tau
)
...
init_classical <- function() {
  list(
    Intercept = 0.5,        # Gaussian location component (identity link)
...
qs_save(blmm_exp1_classical_rt, file = "Angele_et_al_2022/blmm_exp1_classical_rt_cogmod.qs2")
