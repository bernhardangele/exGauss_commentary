...
running-head: "Ex-Gaussian Parameterization in brms"
author:
  - name: Jason Geller
...
To demonstrate the issue with fitting the default ex-Gaussian model in {brms}, I simulate data using a comparison adapted from one of my own studies [@geller2025] in R (@R; version 4.6.0). This allows the methodological issue to be illustrated without singling out any particular researchers or journals, although published examples of this interpretive problem do exist.
...
# Tidy an easystats estimate object for display: drop ROPE columns and
# combine the CI bounds into a single "95% CrI" column.
clean_estimate <- function(x, digits = 1) {
  ...
  d[, c(setdiff(names(d), tail_cols), tail_cols), drop = FALSE]
}
```
